...
const TerrainElevationChart = ({ terrainProfile = [], location }) => {
  const chartData = useMemo(() => {
    if (!terrainProfile || terrainProfile.length === 0) {
      // Mock data if no terrain profile
  ...
  return (
    <div style={{
      ...
      <div style={{ 
        ...
        >
          ...
          {[0, 1, 2, 3, 4].map((i) => {
            ...
            const elevation = maxElevation - (i * elevationRange / 4);
            return (
              <g key={i}>
                <line
                  x1={padding}
      ...
      <div style={{
        ...
        <div>Elevation Range: {elevationRange.toFixed(0)}m</div>
        <div style={{ marginTop: '4px' }}>
          Sample Points: {chartData.length}
        </div>
      </div>
    </div>
  );
